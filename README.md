...
小程序端已支持登录、首页选择游戏、按“账号 / 道具”拆分浏览交换、列表默认展示最近 7 天发布的数据、关键词搜索默认查询最近 60 天数据、下拉刷新与触底分页、详情图片预览、从当前分类直接发布资产、道具龙珠分类结构化录入和展示、最多 9 张图片选择与删除、图片上传、在线出价、出价人展示、关注/取消关注信息、个人中心点击进入“我的关注”、被超价站内通知、未读价格变动提醒、我的发布、我的出价和成交记录。提交估价前必须确认出价承诺，成交后由主理人在后台根据线下跟进结果确认完成或取消；小程序成交记录只展示已完成且当前用户为最终最高出价者的记录，支持分页并展示资产名称和成交跟进状态，用户不能自行确认成交或放弃成交。当前阶段不采集手机号、不提供站内私信，通过出价承诺、成交跟进、后台备注和失联限制形成运营闭环。
...
部署前需要准备 `api/.env.example` 中列出的环境变量、执行数据库迁移、配置 HTTPS 证书，并在微信公众平台配置小程序 request、uploadFile、socket 合法域名。无数据库变更的常规发布可在服务器上传 `/opt/auction-platform.tar.gz` 后执行 `scripts/prod-release.sh`，脚本会解压、安装依赖、类型检查、测试、重启 API、健康检查并部署管理后台静态资源；详细注意事项见 `deploy/tencent-cloud.md`，发布记录见 `docs/releases.md`。
