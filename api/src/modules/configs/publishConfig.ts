...
export const FREE_EXCHANGE_PUBLISH_ENABLED_KEY = "free_exchange_publish_enabled";
export const FREE_EXCHANGE_PUBLISH_DISABLED_REASON = "暂未开放自由交换发布";
...
export type FreeExchangePublishConfig = {
  enabled: boolean;
};
...
export async function readFreeExchangePublishConfig(configs: SystemConfigsRepository): Promise<FreeExchangePublishConfig> {
  const items = await configs.list();
  const byKey = new Map(items.map((item) => [item.key, item.value]));

  return {
    enabled: parsePublishEnabled(byKey.get(FREE_EXCHANGE_PUBLISH_ENABLED_KEY))
  };
}
